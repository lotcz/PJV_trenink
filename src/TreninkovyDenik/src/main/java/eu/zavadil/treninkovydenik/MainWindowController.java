...
import eu.zavadil.treninkovydenik.model.Exercise;
...
public class MainWindowController implements Initializable {
    ...
    */
     public void addNewWorkout() {
        ...
        if (d.isPresent()) {
            ...
            try {                          
                ...
                MainApp.getEntityManager().persist(w);
                ...
                if (lastWorkout != null) {
                    lastWorkout.getExercises().forEach((oldExercise) -> {
                        ...
                        MainApp.getEntityManager().persist(newExercise);
                    });
                }                        
                MainApp.getEntityManager().getTransaction().commit();
                addWorkoutAndSelect(w);
            } catch (Exception e) {
                MessageDialog.show(e.getMessage());
                MainApp.getEntityManager().getTransaction().rollback();                
            }
        }
    }
    ...
    public void removeWorkout() {
        ...
        if (w != null) {            
            if (MessageDialog.showYesNoQuestion("Opravdu si přejete smazat tento trénink?")) {                                
                ...
                try {            
                    workouts.remove(w);
                    MainApp.getEntityManager().remove(w);
                    MainApp.getEntityManager().getTransaction().commit();                   
                } catch (Exception e) {
                    MessageDialog.show(e.getMessage());
                    MainApp.getEntityManager().getTransaction().rollback();                    
    ...
    public void addNewExercise() {
        ...
        if (e != null) {            
            ...
            try {         
                Workout w = getSelectedWorkout();
                e.setWorkout(w);                
                MainApp.getEntityManager().persist(e);
                MainApp.getEntityManager().getTransaction().commit();                
    ...
    public void editExercise() {
        ...
        if (e != null) {
            e = EditExerciseDialog.show(MainApp.getPrimaryStage(), e);
        ...
        if (e != null) {            
            if (MessageDialog.showYesNoQuestion("Opravdu si přejete odebrat tento cvik z tréninku?")) {
                ...
                try {
                    ...
                    MainApp.getEntityManager().remove(e);
                    MainApp.getEntityManager().getTransaction().commit();                    
                    refreshWorkoutForm();
                } catch (Exception ex) {
                    MainApp.getEntityManager().getTransaction().rollback();
    ...
    private void handleEditWeightsButtonAction(ActionEvent event) {
       WeightsDialog.displayDialog(MainApp.getPrimaryStage());
    }
    
    @FXML
    private void handleEditExerciseTypesButtonAction(ActionEvent event) {
       ExerciseTypesDialog.displayDialog(MainApp.getPrimaryStage());
    }
